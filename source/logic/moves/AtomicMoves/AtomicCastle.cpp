...
AtomicCastle::AtomicCastle() = default;
std::shared_ptr<AtomicCastle> AtomicCastle::GetMove() {
  if (castle_ == nullptr) {
    castle_ = std::shared_ptr<AtomicCastle>(new AtomicCastle());
  }
  return castle_;
}
void AtomicCastle::MakeMove(int from_row, int from_col, int to_row, int to_col, Position& position) const {
...
bool AtomicCastle::IsValid() const {
  return true;
}
